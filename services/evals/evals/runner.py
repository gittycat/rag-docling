...
class EvaluationRunner:
    ...
    async def run(
        ...
        if self.config.cache.query:
            # Both guards refuse rather than serve: a cached answer that cannot be
            # attributed to a specific pipeline is worse than no cache at all.
            ...
            elif config_snapshot.prompt_fingerprint is None:
                # An edit to `prompts.context` changes every answer while changing
                # nothing else in this key. A server that does not report its
                # prompts cannot be told apart from the same server after such an
                # edit, so its answers are not cacheable across runs.
                logger.warning(
                    "[EVAL] Query cache disabled for this run: the server did not "
                    "report a prompt fingerprint, so a prompt change would silently "
                    "reuse answers generated by the previous prompt."
                )
                self.config.cache.query = False
            else:
                self._query_cache_key = config_fingerprint(
                    {
                        ...
                        "prompt_fingerprint": config_snapshot.prompt_fingerprint,
                    }
                )
    ...
    def _create_config_snapshot(
        ...
        return ConfigSnapshot(
            ...
            chunker=chunker,
            prompt_fingerprint=rag_config.get("prompt_fingerprint"),
        ...
        if self.config.metrics.citation or self.config.metrics.groundedness:
            ...
            if citation_scope != "explicit":
                scorecard.notes.append(
                    ...
                    "never emits inline citation markers, so citation_precision, "
                    "citation_recall, section_accuracy, citation_entailment, "
                    "claim_citation_support and uncited_claim_rate are undefined for "
                    "this run and the citation objective is dropped from the "
                    "weighted score. Set eval.citation_scope: explicit in config.yml."
                )
