...
| [rag-pipeline.md](rag-pipeline.md) | ingestion, docling, chunking, embedding, contextual retrieval, task worker, chromadb, upload | Document ingestion end to end: parsing, chunking, contextual prefixes, embeddings, the `SKIP LOCKED` task queue |
| [retrieval.md](retrieval.md) | retrieval, search, bm25, vector, hybrid, rrf, fusion, reranker, cross-encoder, top_k | Hybrid BM25 + vector retrieval, RRF fusion, cross-encoder reranking, retrieval knobs and failure modes |
...
| [database.md](database.md) | database, postgres, schema, tables, connection pool, sqlalchemy, queries, skip locked | Schema, indexes, connection pooling, query patterns, the task queue |
