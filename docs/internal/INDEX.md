...
| [configuration-reference.md](configuration-reference.md) | config, configuration, yaml, config.yml, secrets, env vars, precedence, dead config, data_policy, execution_boundary | Exhaustive `config.yml` reference, environment variables, Docker secrets, compose overlays, precedence, dead keys |
| [pii-masking.md](pii-masking.md) | pii, masking, privacy, presidio, spacy, gliner, gdpr, anonymization, data policy, execution boundary, judge egress | PII detection and masking, threat model, coverage boundary, validation, guardrails, audit logging, boot-time refusals, and the separate `data_policy` judge-egress gate |
...
| [design-decisions.md](design-decisions.md) | why, decisions, rationale, postmortem, nullpool, history, tradeoffs | The reasoning behind key choices: in-house evals, the async/NullPool incident, Docling constraints, cache bounds, the PII tier boundary, the execution-boundary data policy |
