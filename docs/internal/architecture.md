...
| `tei` | Self-hosted embedding inference (HuggingFace Text Embeddings Inference, serving `Qwen/Qwen3-Embedding-0.6B`) | Yes | Backs the `tei` provider in `config.yml`; no port published to the host |
...
- **`public`**: host-reachable. `webapp`, `rag-server`, `task-worker`, and `evals`
  sit on it — `rag-server` and `task-worker` need it to reach cloud LLM
  providers (OpenAI, Anthropic, etc.) over the internet. Embedding inference
  does not need this: `tei` is reached over `private` at `http://tei:80`, the
  in-compose service DNS name.
- **`private`**: `internal: true` — no default gateway, no internet access, fully
  ...
  `private`). `tei` also sits on `private` — that's the network the app talks
  to it over — but is additionally dual-homed on `public`, needed only for its
  own one-time egress to `huggingface.co` to download model weights on a cold
  volume. Nothing is published from `tei` on either network.
...
| `docker-compose.cloud.yml` | Yes | Replaces local `build:` with `image:` pulls for cloud deployment |
...
3. The LLM (a cloud provider, or a self-hosted `vllm` endpoint) generates the
   answer, optionally streamed over SSE.
...
per vector at 768 dimensions against about 3.2 KB — which lifted the same
machine to roughly 765k documents. (768 was the dimension in place at the time
this was measured, from Ollama's `nomic-embed-text`; the active embedding
model is `Qwen/Qwen3-Embedding-0.6B` via TEI at 1024 dimensions today, which
raises the per-vector RAM cost proportionally — not remeasured.)
...
OpenAI, Anthropic, and vLLM (OpenAI-compatible, self-hosted — the local-inference
option now that Ollama is gone). Embeddings are served separately by TEI (`tei`
provider) or OpenAI.
