...
- **`HealthTab`** — the default landing tab. Fetches the two most recent eval runs, derives a "weakest link" verdict banner across retrieval/generation/citation/abstention metric groups, and renders stat panels for retrieval recall (the emphasised top-left panel, showing the funnel's final recall@5 with its candidate-list ceiling beneath), retrieval bottleneck, generation, cost per query, and latency p95, each with a delta against the previous run. Composes `RetrievalFunnel`, `ConfigContext`, `LatencyPanel`, and `MetricBreakdown`.
...
- **`RetrievalFunnel`** — recall@5 as a bar per pipeline stage (BM25 and vector, which run in parallel, then RRF fusion, then rerank), each with its delta against its *real* predecessor: the legs carry none, fusion is scored against the better leg, rerank against the candidate list it was handed. Underneath, the two ways evidence is lost — "never retrieved" (`1 - ceiling`) and "dropped by rerank" (`ceiling - final`) — and a sentence naming the larger as the bottleneck with an action. Renders its own `note` instead of bars when the run has no per-stage scores (a generation-tier run, or questions with no resolvable gold), never a zero.
